...
describe('Função que recebe a temperatura em graus Farenheit e retorna em graus Celsius.', function() {
  function farenheitToCelsius(farenheit) {
    return ((farenheit - 32) / 1.8); // IMPLEMENTE
  }
...
describe('Função que recebe um vetor, e retorna quantas vogais foram encontradas em ordem', function() {
  function vogais(arr) {
    var vogaisArr = []

    arr.forEach(letra => {
      if(letra == 'a' || letra == 'e' || letra == 'i' || letra == 'o' || letra == 'u') {
        vogaisArr.push(letra)
      }
    })

    return vogaisArr.sort(); // IMPLEMENTE
  }

  it('retorna o array de consoantes teste um', function() {
    expect(vogais(['a', 'b', 'c'])).toEqual(['a']);
  });

  it('retorna um array vaziu se nenhuma consoante for encontrada', function() {
    expect(vogais(['b', 'x'])).toEqual([]);
  });

  it('retorna o array de consoantes teste três', function() {
    expect(vogais(['a', 'e', 'i', 'x', 'o', 'u'])).toEqual(['a', 'e', 'i', 'o', 'u']);
  });

  it('retorna o array de consoantes teste quatro', function() {
    expect(vogais(['e', 'f', 'i', 'g', 'a', 'b', 'c', 'd'])).toEqual(['a', 'e', 'i']);
  });
});
